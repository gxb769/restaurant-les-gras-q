...
export default function Gallery({ dict }: Props) {
  ...
  return (
    <section id="galerie" className="bg-paper py-[104px]">
      <div className="w-[min(1180px,calc(100%-40px))] mx-auto">
        ...
        {/* Photo grid — 3 cols on desktop, stacked on mobile */}
        <div className="grid grid-cols-3 grid-rows-2 gap-3 h-[580px] max-md:grid-cols-2 max-md:h-auto max-sm:grid-cols-1">
          {photos.map((p, i) => (
            <ScrollReveal
              key={p.src}
              delay={i * 0.07}
